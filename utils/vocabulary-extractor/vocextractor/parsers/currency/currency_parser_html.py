...
@register.parser
class CurrencyParserHTML(VocabularyParser):
    ...
    def vocabulary(self) -> Vocabulary:
        ...
        Returns:
            Vocabulary data
        """
        return Vocabulary.create(name='Currency vocabulary',
                                 description='Currency values for socio-economic datasets',
                                 url='http://publications.europa.eu/resource/authority/currency',
                                 topic='CURRENCY')
    ...
    def label(self, row) -> str:
        ...
        end_date = row.find('td', {"class": "yui3-datatable-col-endDate"})
        date_span = end_date.findChild('span')
        date = date_span.getText().split('/')
        if len(date) > 2: label = label + "(until " + date[2] + ")"
        return label
